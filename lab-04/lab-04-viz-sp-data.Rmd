...
```
```{r fig.margin = TRUE, echo = FALSE, eval = TRUE, fig.alt = "Photo of comedian Mitch Hedberg performing stand-up comedy."}
...
- `states$abbreviation` is the list of US state abbreviations you already loaded (including DC) — use this instead of R's built-in `state.abb` constant, which leaves DC out and would mislabel any DC location as a non-US country
...
```{r filter-us-laquinta, eval = TRUE}
laquinta <- laquinta %>%
  filter(country == "United States")
```

**Notice the `laquinta <-` here too** — same reasoning as Exercises 6 and 8. Without it, this filtered version would print once and disappear, and every exercise below would silently keep working with the *unfiltered* dataset (including the Canada/Mexico/Colombia locations from Exercise 8).
...
```{r plot-all-locations, eval = TRUE, fig.alt = "Scatterplot of Denny's and La Quinta locations across the United States, colored by establishment, with the points roughly outlining the shape of the country."}
...
```{r nc-plot, eval = FALSE, fig.alt = "___"}
# Remember to change eval=FALSE to eval=TRUE!
# Also fill in fig.alt above with a short description of what your finished plot shows
dn_lq %>%
  filter(___ == "NC") %>%
...
```{r texas-plot, eval = FALSE, fig.alt = "___"}
# Remember to change eval=FALSE to eval=TRUE!
# Also fill in fig.alt above with a short description of what your finished plot shows
...
- **DC locations showing up with a missing/NA country** → If you used R's built-in `state.abb` instead of `states$abbreviation` in Exercise 8, DC will fall through `case_when()` uncaught, since `state.abb` doesn't include it
...
**✓ Final Checkpoint:** Visit your GitHub repo one more time to confirm all your work is there — this is your checkpoint/backup. Then double-check your downloaded PDF is complete and readable before uploading it to Canvas. **The PDF is the basis for grading.**
