...
**If you haven't completed these yet, take care of them before continuing** — the rest of this lab assumes they're done.
...
```{r yaml-raw-to-rendered, fig.fullwidth=TRUE, echo = FALSE, fig.alt="Screenshot comparing the raw YAML header text in an R Markdown file to how the title and author render at the top of the knitted document."}
...
```{r update-author-name-commit, fig.fullwidth=TRUE, echo = FALSE, fig.alt="Screenshot of the RStudio Git commit dialog, showing the changed file checked, the diff view, a commit message box with 'Update author name' typed in, and the Commit button."}
...
```{r fig.fullwidth=TRUE, fig.alt="Scatterplot of y versus x for the dino dataset, with points forming the outline of a dinosaur."}
...
```{r star-plot, fig.fullwidth=TRUE, eval = FALSE, fig.alt="UPDATE THIS: describe what your star-dataset scatterplot shows, for screen-reader accessibility."}
...
- The correlation coefficient for the star dataset is: [type your answer here]

- Compared to the dino dataset (r = -0.06447185), the star dataset's correlation is: [type your answer here]
...
```{r circle-plot, fig.fullwidth=TRUE, eval = FALSE, fig.alt="UPDATE THIS: describe what your circle-dataset scatterplot shows, for screen-reader accessibility."}
# Your code here
```
...
- The correlation coefficient for the circle dataset is: [type your answer here]

- Compared to the dino dataset, the circle dataset's correlation is: [type your answer here]

- What do you notice about the correlation coefficients for dino, star, and circle? [type your answer here]
...
```{r all-viz, eval=FALSE, fig.fullwidth=TRUE, fig.alt="Grid of scatterplots, one per dataset in the Datasaurus Dozen, arranged in 3 columns and colored by dataset, with no legend."}
...
```{r fig-resize-global, fig.margin = TRUE, echo = FALSE, fig.width=3, fig.alt="Screenshot of the R Markdown 'Output Options' dialog with the Figures tab open, showing height and width fields for global figure sizing."}
...
```{r fig-resize-local, fig.margin = TRUE, echo = FALSE, fig.width=3, fig.alt="Screenshot of a code chunk's gear icon menu in RStudio, showing fig.width and fig.height options for sizing a single figure."}
...
```{r theme-highlight, fig.margin = TRUE, echo = FALSE, fig.width=3, fig.alt="Screenshot of the R Markdown 'Output Options' General tab, showing dropdown menus for syntax highlighting and document theme."}
...
Every lab has two required parts, both due by the deadline:

1. **Push to GitHub** — your completed `.Rmd`, committed and pushed (covered above)
2. **Upload a PDF to this lab's Canvas assignment** — see below
...
2. Select **Knit to PDF** (this uses the Tufte Handout format defined in the YAML)
3. Wait for the PDF to generate
4. Find `lab-01-hello-r.pdf` in the **Files** pane, check its box, then **More → Export...** to save it to your computer
5. Upload that PDF to this lab's Canvas assignment

**Troubleshooting:**

- **"LaTeX not found" or "pdflatex not found":** Run `tinytex::install_tinytex()` in the R console, restart RStudio, and try knitting to PDF again
- **Still not working?** Knit to HTML instead and use your browser's **Print → Save as PDF** option — that's a completely acceptable submission

**If only one part works:** if you get the GitHub push or the PDF working but not both after trying the steps above, submit what you have by the deadline and email me — I'd rather see your progress than have a technical snag cost you credit.

**Remember:** Submit BOTH to GitHub (your `.Rmd`) AND Canvas (the PDF). Both are required.
