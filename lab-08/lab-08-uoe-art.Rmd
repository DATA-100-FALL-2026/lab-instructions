...
**For this lab:** The University of Edinburgh allows scraping of this public collection, but we should still be respectful. We'll add delays between requests to avoid overwhelming their server, and we'll scrape a generous but reasonable sample of pages rather than the entire collection.
...
Your histogram from Exercise 11 probably shows a small cluster of pieces that look much older than the rest of the collection. Before assuming this is a data error, let's investigate it - not every extreme value in a dataset is a mistake!

**Filter for the oldest pieces in the collection:**
```{r find-old-pieces, eval = FALSE}
# Remember to change eval=FALSE to eval=TRUE!
# Pick a reasonable cutoff based on where your histogram's tail begins
uoe_art %>%
  filter(year < ___) %>%
  arrange(year)
```

**Visit the links for two or three of these oldest pieces on the art collection website.**

**Are these data errors, or are they legitimate pieces? What are they?** (Hint: look closely at the titles - many of them describe the same kind of object.)



**Why would a 20th/21st-century art college's collection contain objects with dates this old?**
...
🧶 ✅ ⬆️ **Knit, commit with message "Completed Exercise 13", and push your changes.**
